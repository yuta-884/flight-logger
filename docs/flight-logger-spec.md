...
1. **slugの仕様**: §11で決定
2. **既存flight-log（個人版）との関係**: **判断を保留**。flight-logger MVP（Phase 1〜4程度）を実装し使用感を確認してから、併存か・個人データをflight-loggerに移行して一本化するかを決める

（解決済み: データ提供元と保存モデル → Option A確定、§10）

## 11. slug仕様

公開プロフィール `/u/{slug}` と埋め込みカード `/embed/{slug}` のURLに使う識別子。決めるべき論点と決定:

| 論点 | 決定 |
|---|---|
| 文字種・形式 | 小文字英数字とハイフン（`[a-z0-9-]`）、先頭は英字、連続ハイフン・末尾ハイフン不可、3〜30文字。入力は小文字に正規化 |
| 一意性 | 大小無視で一意（DBは `citext` または小文字正規化して `unique`） |
| 予約語 | システムルートと衝突する語をブロック: `admin, api, embed, u, login, logout, settings, about, help, static, assets, public, new, edit` 等。攻撃的な語も簡易ブロック |
| 初期設定 | **初回ログイン時にユーザーが自分で入力**（オンボーディングで必須）。設定完了までアプリ本体に進めない |
| 変更可否 | **いつでも変更可**。旧slugは即時解放され再利用可能（他人が取得しうる）。旧公開URLはリダイレクトを持たず失効する |

- 変更可のため、公開URLは「安定リンク」ではない点をUI上で明記（変更すると旧URLの共有が切れる旨を注意表示）
- 実装: `profiles.slug` を小文字正規化して `unique`。予約語・形式はサーバー側（Edge FunctionまたはDB制約/トリガ）で検証し、クライアントでも即時バリデーション
