...
- **§11-A 初期slug**: (a) 初回ログイン時にユーザーが自分で入力して設定 / (b) Googleの表示名やメールローカル部から自動生成し後で変更可能にする
- **§11-B 変更可否**: (a) 変更不可（公開URLが安定） / (b) いつでも変更可（旧URLは失効。リダイレクトは持たない） / (c) 変更可だが旧slugは再利用不可で予約
- 文字種・予約語・一意性は上記で確定。A/Bのみ要ユーザー判断
