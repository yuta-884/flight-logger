# flight-logger 要件・仕様書（v0.3 ドラフト）
...
2. **AeroDataBox BASICプランの規約確認**: 複数人が使うサービスでの利用可否（商用ではないが共用になる）
   - 調査メモ（2026-07-13、公式サイト・検索結果より）: BASIC=600ユニット/月・1req/s。公式は自らを「小規模な旅行・アビエーション系アプリ、小チーム、個人開発者向け」と位置づけており、**小規模マルチユーザーアプリは想定利用範囲内とみられる**。教育・研究向け無償キーには目的外利用禁止の明確な制限があるが、通常プランの同種の条項は本文未確認
   - **要確認（残タスク）**: Terms of Use本文の (a) 取得データの永続保存・再配布可否（本アプリはDBに保存し公開プロフィールで第三者に表示するため）、(b) 無料プランの商用利用条項。aerodatabox.com のToUを直接確認すること
