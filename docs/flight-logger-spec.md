...
**MVP（Phase 1〜5）は実装済みで、ローカル環境で動作検証済み**（§8）。本書は実装済みの現行仕様を記述する。本番デプロイは未実施（§9）。
...
| ホスティング | **Cloudflare Pages**（無料。プレビューデプロイ・帯域無制限を評価して決定）。デプロイは未実施 |
...
1. **本番デプロイ**: GitHubリモート作成 → Cloudflare Pagesデプロイ。Supabase Auth・Google OAuthへの本番URL追加等のセットアップが必要
2. **既存flight-log（個人版）との関係**: 判断を保留中。MVPが使える状態になったので、使用感を確認してから併存か・個人データをflight-loggerに移行して一本化するかを決める
