...
| カード画像ダウンロード | 埋め込みカードをPNG（1280px幅・2倍解像度・角丸外は透過）で保存できる。カード全体をcanvasで自前描画（`src/lib/cardCanvas.ts`。DOMキャプチャ系はiOS Safariで白画像になるため不採用）。導線は `/embed/{slug}` 直接表示時のボタン（iframe内では非表示）と、Settings共有カードのボタン（`?download=1` 付きで埋め込みページを開き自動保存） |
