...
- 本番URL: https://flight-logger.nayuta-labs.workers.dev
...
| 埋め込みカード | `/embed/{slug}`。公開ONのユーザーのみ。**飛行日誌（ログブック）型**のカードで、航路図・年ごとの飛行実績（年/便数/距離）・合計欄・国旗を1枚にまとめる。iframeで外部サイトに埋め込む想定。表示・画像出力とも `src/lib/cardCanvas.ts` の同一関数で描画する |
...
| カード画像ダウンロード | 埋め込みカードをPNG（1280px幅・2倍解像度・角丸外は透過）で保存できる。DOMキャプチャ系はiOS Safariで白画像になるため使わず、canvasに自前描画する。画面表示は同じ描画関数をそのままcanvas要素へ描き（PNG符号化しないぶん高速）、ダウンロード時のみオフスクリーンでPNG化するため、表示と保存画像が食い違わない。導線は `/embed/{slug}` 直接表示時のボタン（iframe内では非表示）と、Settings共有カードのボタン（`?download=1` 付きで埋め込みページを開き自動保存） |
| カードデザインの経緯 | 当初はflight-log由来のパスポート風カード（MRZ・発行欄・円形国旗）だったが、Flightyの共有カードと構造・細部が酷似しており、利用者層が重なるため一般公開にあたり刷新した。地球儀型・ミニマル型と比較検討のうえ、**裏側のルートが欠けない**平面図を持つログブック型を採用（2026-08-01） |
| 使い方ガイド | `/guide`。認証不要の公開ページ（日英対応）。登録〜検索/手入力/Flightyインポート〜滞在チェック〜共有までを、Flighty未経験者にも分かる前提で説明。導線は全画面共通フッター＋ログインカード内 |
