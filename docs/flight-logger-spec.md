...
| **FlightAware AeroAPI** | ✅ **保存可**（rawは30日まで／派生データ(Derivative Works)は無期限）・**個人/B2Iアプリ可**・**第三者提供可**（rawでなく製品に組込む形なら） | ✅ 良好（グローバル、2011年以降の履歴） | ✅ 2011年〜 | △ Personalに少額の無料クレジット（~$5/月）、以降従量。要CC登録 |
| **AviationStack** | △ サブスク中のキャッシュは可 | △ | ❌ **履歴は有料のみ**（無料枠はリアルタイムのみ） | △ 無料=リアルタイムのみ |
| **Amadeus Self-Service** | △ | ❌ **LCC非対応**（AA/Delta/BA/LCCを返さない） | △ | 無料テスト枠あり |

**判明した要点**:
- **FlightAware AeroAPI** は規約上、本プロダクトの用途をほぼ明示的に許諾: 「Search, retrieve, download and **store** AeroAPI Data」「Use...for **personal purposes and projects**」「Incorporate...in a **business-to-individual consumer application**（例: アプリストアで個人向けに提供するフライトトラッキングアプリ）」「**Provide AeroAPI Data to third-parties**（rawでなく製品に組込む形／追加課金なし／Permitted Purposes限定）」。制約は「**rawデータの保存は30日まで**（派生データ＝加工後のログは無期限可）」「帰属表示 "Contains AeroAPI data © FlightAware LLC [year]" が必要」。→ **Option Bの有力候補**。加工済みログ（区間・日付・算出距離・統計）は「Derivative Works」として無期限保存・表示でき、公開プロフィールも「製品への組込み表示」として整合する
- **AviationStack**: 過去便のログには履歴照会が必須だが履歴は有料のみ。無料$0運用と両立しない
- **Amadeus**: LCC非対応が致命的（本ユーザーのデータの大半がLCC）
- 補足: 現行の AeroDataBox は**コスト確実に$0だが規約が保存・公開表示を許さない**。FlightAware は**規約は用途に合致するが従量課金（少額無料クレジット内に収まる見込みだがCC登録必須・要金額確認）**
...
| コスト | ◎ 確実に$0 | ○ ~$5/月クレジット内の見込み（要確認・CC必須） |
| 保持できるデータ | 事実＋ローカル計算値のみ（opsなし・実飛行時間なし） | 加工後の運航データも保持可（機材・実時刻→飛行時間も算出可） |
| 実装 | シンプル（レスポンス破棄） | 30日rawルール・帰属表示の遵守が必要 |
| 移植 | flight-logのadd_flight解決ロジックほぼ流用 | 解決処理をFlightAware用に書き換え |

**推奨**: どちらも成立する。**コストの確実性と実装の単純さを最優先するなら A**。**運航データ・実測飛行時間を残し、公開表示を規約上クリーンにしたいなら B（FlightAware）**。要ユーザー判断（§9-1）。
