# flight-logger 要件・仕様書（v0.2 ドラフト）
...
| フロントエンド | 静的サイトとして配信。統計・地球儀はクライアント側で計算（flight-logのロジック移植） |
...
1. **フロントの構成**: 推奨はVite＋React＋TypeScript（§9.1参照）
2. **slugの仕様**: 文字種・変更可否・予約語（admin等）
3. **AeroDataBox BASICプランの規約確認**: 複数人が使うサービスでの利用可否（商用ではないが共用になる）
4. **既存flight-log（個人版）との関係**: 併存か、自分のデータをflight-loggerに移行して一本化するか

### 9.1 フロント構成の検討メモ

flight-logがvanillaなのは「1枚の静的ページ」だから成立した選択。flight-loggerは認証状態・フォーム・一覧CRUD・複数ルートを持つ「アプリ」であり、コンポーネントフレームワークの採用が妥当。統計・地球儀・カードのロジックはフレームワーク非依存の純粋なJSモジュールとして移植するため、どの選択でも流用できる。
